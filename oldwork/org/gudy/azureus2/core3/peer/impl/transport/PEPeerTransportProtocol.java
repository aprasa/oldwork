...
import org.gudy.azureus2.core3.peer.impl.transport.client.GetPort;
import org.gudy.azureus2.core3.peer.impl.transport.client.GetInfo;
import org.gudy.azureus2.core3.peer.impl.transport.client.UserDN;
import org.gudy.azureus2.core3.peer.impl.transport.client.TargetDN;
import org.gudy.azureus2.core3.peer.impl.transport.torrent.VerifierFile;
import org.gudy.azureus2.core3.peer.impl.transport.torrent.AddFileWindow;
...
import org.gudy.azureus2.core3.torrent.impl.SecureTorrent;
...
implements PEPeerTransport
{
    ...
    public static boolean NotYet=false,Premier=false,PremierUserDN=true,PremierTargetDN=true,PremierPermisDecision=true,PremierFileDN=true,PremierAddRoleToLDAP=true,PremierAddTorrentFile=true;
    public static String[] tab=new String[20];
    public static String LastPassed="",LdapPassword;
			...
			if (Logger.isEnabled())
			    Logger.log(new LogEvent(this, LOGID,
			    		"Sending handshake with reserved bytes: " +
			    			ByteFormatter.nicePrint(handshake.getReserved(), false)));
			System.out.println("*** Sending BTHandshake");
	...
	///////////////                                      
	//////////////////////////////////////////////////////////////////////////////////////////////////////////
	/// if i am trying to download, i listen if the file that i am trying to download is protected
	// if yes,i will receive an information from the client telling me to type my id and password
	//But if someone with IP=ip is come to download a file, there is two ways to manage:
	//1. if the client is not modified, i refuse all his requests, ie: i do not send him back my handshake
	//2. if the client is modinfed, i launch my socket to authentificate him, if he is well authentificated, i pass
	// to the second and final step: call permis decision engine the get the authorisation decision of the ressource.
	//if not authentificated, throw out his request, ie: do not send back az handshake
	//
	////////////////////////////////////////////////////////////////////////////////////////////////////////////

	private void sendAZHandshake() {
		
		boolean seeding=manager.isSeeding();
		System.out.println("\n\nseeding:   "+seeding);
...
//		
		...
		//GetPort m=new GetPort("127.0.0.1","TITANIC");
//lancer GetPort
/*if(seeding)// if they come to download from me
		
{
	
	
	//GetPort m=new GetPort(NewIP,Titre);
			GetPort m=new GetPort("127.0.0.1",Titre);
	
	//check if the client is modified
	boolean isServerListening=GetPort.Connected();
	
	//check if the client is authentified
	boolean Authentificated=GetInfo.isAuthentificated();
	
	
	//quit if the client is not modified
	if (isServerListening) //if no server listening
	{
		System.out.println(" \n\n........no server listening ");	
	}
	...
	if (Authentificated) //if authentificated, call permis decision engine
		
				{// get the user name
		        String UserName=GetInfo.UserName();
		        // get the file CN
		        String FileCN=Titre;
				//get the full user DN
				 
					String NameDN=UserDN.getDN(UserName);
				// get the full target DN
					
		
				String Target=TargetDN.getDN(FileCN);
					...
					boolean reponse=issrg.aef.SampleAEF1.Permis(NameDN,Target);
					System.out.println(" \n\n\n\n\n\n\n\n Response of PERMIS:");
					...
					if(!reponse)
					{
							...
							// do not send back the az handshake
						System.out.println("    "+ UserName+   "    can not download the file :"+FileCN);
		
					}
					else {
							System.out.println("    "+ UserName+   "    can download the file : "+FileCN);
							System.out.println("\n\nSending AZHandshake to :"+NewIP+"\n\n");
	...
	//else connection.getOutgoingMessageQueue().addMessage( az_handshake, false );
...
//end if (seeding)  
 ...
 //if i am the downloader, send the handshake emmidiately
	    System.out.println("\n\nSending AZHandshake to :"+NewIP+"\n\n********************************************\n\n"); 
	...
	public void sendHave( int pieceNumber ) {// we desactivated the have message to controle the requests
	...
	protected void decodeBTHandshake( BTHandshake handshake ) {
		...
		//while(org.gudy.azureus2.core3.peer.impl.transport.PEPeerTransportProtocol.NotYet)
		//{
		...
		System.out.println("\n\nFile name:    "+DISPLAYNAME+"\n\n");
		 ...
		 byte[] dd=manager.getHash(),
		 
		 
		 //le byte[] a partir du bthandshake
		 ff=handshake.getDataHash();
		 
		 
		 //convertir ce byte array en String
		 String TheClearHash=org.gudy.azureus2.core3.util.ByteFormatter.nicePrint( ff );
		 
		 
		 System.out.println("\n\n***HASH FROM HANDSHAKE: "+ TheClearHash);
		 String TheClear=org.gudy.azureus2.core3.util.ByteFormatter.nicePrint( dd );
		 
		 System.out.println("***HASH FROM MANAGER:   "+ TheClear+"\n\n");
			int lo=dd.length,li=ff.length; int l=0;
			for (l=0;l<lo;l++)
			{
				mana=mana+dd[l];
			//System.out.print(" "+dd[l]);
			}
			//System.out.println("hache = "+mana);
			//System.out.print("\n\n");
			...
			{
				hand=hand+ff[l];
			//System.out.print(" "+ff[l]);
			}
			//System.out.println("hach = "+hand);
			String value=new String(handshake.getDataHash());
		...
		{
			boolean AmSeeding=manager.isSeeding();
			
			System.out.println("\nAm Seeding= "+AmSeeding);
			System.out.println("seed= "+isSeed());
			
			
			//verifier si le fichier est public,si oui:pas de controle d'acces
			if(PremierPermisDecision)
			{
				PremierPermisDecision=false;


			FileDN cd=new FileDN();
			FileDN.go();

			}
			FileDN.getDN("cn="+TheClearHash);
			boolean  isPuplic=FileDN.isPublic();
			
			//si le fichier n'est pas public, faire l'auth+autorisation
			if(!isPuplic)
			{
			...
			if(AmSeeding)//it they come to download from me
			{
				//receive the informations
				GetPort m=new GetPort(NewIP,DISPLAYNAME);
...
//				check if the client is modified
				...
				//quit if the client is not modified
				if (isServerListening) //if no server listening, destroy the bthandshake
				{
					System.out.println(" \n\n........no server listening \n");
				...
				if (!Authentificated) //destry the bthandshake if the client is not authentificated
				{
				...
				if (Authentificated) //if authentificated, call permis decision engine for authorisation
					
							{// get the user name
					        String UserName=GetInfo.UserName();
					        // get the file CN
					        String FileCN=TheClearHash;
							//get the full user DN
					        ...
					        UserDN.go();
					           ...
					           String NameDN=UserDN.getDN("cn="+UserName);
							// get the full target DN
					           if (PremierTargetDN)
						        {
						        	PremierTargetDN=false;

						        
						       
						        TargetDN ppp=new TargetDN();
						        TargetDN.go();

						        }
					
							  String Target=TargetDN.getDN("cn="+FileCN);
					...
					//ce ke j'ai eliminer
								//boolean reponse=PermisDecision.getDecision(NameDN,FileCN);
								//FileDN.reset();
							  ...
							  boolean reponse=issrg.aef.SampleAEF1.Permis(NameDN,Target);
								...
								//boolean reponse=issrg.aef.SampleAEF1.Permis("cn=Mestafa,ou=Famille,o=Membres,c=gb","ou=Famille,o=Fichiers,c=gb");
								System.out.println(" \n\n\n\n\n\n\n\n Response of PERMIS:");
								System.out.println(                  " ___________________\n\n");
								System.out.println("________________________________________________________\n");
								if(!reponse)//destroy the bthandshake if permis says no
								{
										...
										// do not send back the az handshake
									System.out.println("    "+ UserName+   "    can not download the file :"+DISPLAYNAME);
									System.out.println("________________________________________________________\n");
									closeConnectionInternally( "handshake has wrong infohash" );
									handshake.destroy();
									return;
										...
										System.out.println("    "+ UserName+   "    can download the file : "+DISPLAYNAME);
			...
			/////////////////////////////////////////////////////////////////////////////////
			////////   end authentification and authorisation     //////////////////////////
			/////////////////////////////////////////////////////////////////////////////////
			...
			//if(!AmSeeding)  // : i am downloading the file 
				if(true)
			 {
				...
				//boolean isNameExist=VerifierFile.VerifierFile("cn="+DISPLAYNAME);
				// si oui, ne l'ajoute pas
				//si non , ajoute le dans le serveur en interrogeant l'user
				//
				/*if  (ii)
				//((Nom.compareTo(LastPassed)==0) )
				//(Nom.compareTo(LastPassed)>0)
				{
					ii=false;
					System.out.println("**********************************************");
					...
					LastPassed=Nom;
				       ...
				       boolean isNameExist=VerifierFile.VerifierFile("cn="+Nom);
				       if(!isNameExist)
				 
				       {
				    	   
				       
				    	   AddFileWindow st=new AddFileWindow(Nom);
					...
					if(!Exist)
					{
						tab[i]=Nom;
					       ...
					       /////////////////////////////////////////////
							...
							//if(AmSeeding) {GetPort m=new GetPort("127.0.0.1",DISPLAYNAME);}
					       ...
					       // add it to the server if not existing
					       if(!isNameExist)
					 
					       {
					    	   
					       
					    	   //AddFileWindow st=new AddFileWindow(Nom,DISPLAYNAME);
					    	   org.gudy.azureus2.core3.torrent.impl.SecureTorrent fen2=new org.gudy.azureus2.core3.torrent.impl.SecureTorrent(Nom,DISPLAYNAME);
...
//////					
		...
		//*************************************************************************************
		//*************************************************************************************
		org.gudy.azureus2.core3.peer.impl.transport.FileDN.isPublic=false;
